...
def get_layers(img, depth_map, div=30):
    ...
    for thres in range(255 - div, 0, -div):
        ret, mask = cv2.threshold(depth_map, thres, 255, cv2.THRESH_TOZERO)
        ret, prev_mask = cv2.threshold(depth_map, prev_thres, 255, cv2.THRESH_TOZERO)
    ...
    # adding last layer
    ret, mask = cv2.threshold(depth_map, 0, 255, cv2.THRESH_TOZERO)
    ret, prev_mask = cv2.threshold(depth_map, prev_thres, 255, cv2.THRESH_TOZERO)
...
def conv_cv_pygame_alpha(cv_image):
    tmp = cv2.cvtColor(cv_image, cv2.COLOR_RGB2GRAY)
